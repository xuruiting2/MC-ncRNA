...
#            head_mask = [None] * self.config.num_hidden_layers
        ...
        # return outputs  # sequence_output, pooled_output, (hidden_states), (attentions),tensor_emb
        return outputs  # sequence_output, pooled_output, (hidden_states), (attentions),tensor_emb
...
class AttentionFusion(nn.Module):
    def __init__(self, dim1, dim2, hidden_dim):
        ...
        # Linear layers to project features to a shared hidden space
        self.proj1 = nn.Linear(dim1, hidden_dim).to(device)
        self.proj2 = nn.Linear(dim2, hidden_dim).to(device)

        # Attention mechanism
        self.attn_weights = nn.Linear(hidden_dim, 1).to(device)

    def forward(self, feature1, feature2):
        # Project features to a shared hidden space
        proj1 = F.relu(self.proj1(feature1))
        proj2 = F.relu(self.proj2(feature2.float()))
        # Compute attention scores
        attn_scores = self.attn_weights(proj1 + proj2)

        # Apply softmax to get attention weights
        attn_weights = F.softmax(attn_scores, dim=1)

        # Apply attention weights to the original features
        fused_feature = (attn_weights * feature1) + ((1 - attn_weights) * feature2)

        return fused_feature

class WeightFusionModel(nn.Module):
    def __init__(self, gene_feature_dim, disease_feature_dim, hidden_dim):
        super(WeightFusionModel, self).__init__()

        # Linear layers for gene features
        self.gene_linear = nn.Linear(gene_feature_dim, hidden_dim).to(device)

        # Linear layers for disease features
        self.disease_linear = nn.Linear(disease_feature_dim, hidden_dim).to(device)

        # Weight parameter for feature fusion
        self.weight = nn.Parameter(torch.ones(1)).to(device)

    def forward(self, gene_feature, disease_feature):
        # Apply linear transformations to gene and disease features
        gene_hidden = F.relu(self.gene_linear(gene_feature))
        gene_hidden = gene_hidden.to(device)
        disease_hidden = F.relu(self.disease_linear(disease_feature))
        disease_hidden = disease_hidden.to(device)
        # Weighted fusion of gene and disease features
        fused_feature = self.weight * gene_hidden + (1 - self.weight) * disease_hidden
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        pooled_output = outputs[1]
        info_fea = torch.tensor(info_fea).to(device)
        fusion_attention = AttentionFusion(768,768,768)
        fusion_fea = fusion_attention(pooled_output,info_fea)
        fusion_fea = self.dropout(fusion_fea)
        seq_feature = pooled_output / pooled_output.norm(dim=1, keepdim=True)
        graph_feature = info_fea / info_fea.norm(dim=1, keepdim=True)  
        logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.01)).to(device)
        fu_feature = logit_scale.float() * seq_feature.float() @ graph_feature.t().float()
        t_feature = fu_feature.t()
        contr_labels = torch.arange(fu_feature.shape[0]).long().to(device)
        total_loss2 = (F.cross_entropy(fu_feature, contr_labels) + F.cross_entropy(t_feature, contr_labels)) / 2
        logits = self.classifier(fusion_fea.float())
        outputs = (logits,) + outputs[2:]  # add hidden states and attention if they are her
        if labels is not None:
            if self.num_labels == 1:
                 #  We are doing regression
                loss_fct = MSELoss()
                loss = loss_fct(logits.view(-1), labels.view(-1))
                loss = 0.8 * loss + 0.2 * total_loss2
            else:
                loss_fct = CrossEntropyLoss()
...
class BertContrastive(BertPreTrainedModel):
    ...
    Examples::
        ...
        seq_feature = pooled_output / pooled_output.norm(dim=1, keepdim=True)
        graph_feature = info_fea / info_fea.norm(dim=1, keepdim=True)
        similarity_matrix = torch.matmul(seq_feature, graph_feature.T)
        batch_size = similarity_matrix.shape[0]
        contrastive_labels = torch.arange(batch_size).long().to(similarity_matrix.device)
        margin = 0.1 
        positive_pairs_loss = F.cross_entropy(similarity_matrix, contrastive_labels) 
        negative_pairs_loss = torch.clamp(margin - similarity_matrix.diagonal(), min=0).pow(2).mean()

        total_loss = positive_pairs_loss + negative_pairs_loss

        return total_loss  # (loss), logits, (hidden_states), (attentions)
