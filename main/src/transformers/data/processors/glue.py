...
# 这一块是我新加的，为了处理rna的下游数据
class RnaFamilyProcessor(DataProcessor):
    """Processor for the ncRNA promoter data"""
    def get_labels(self):
    ...
    def get_pre_examples(self, data_dir):
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "pre.tsv")), "pre")
    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, line) in enumerate(lines):
...
# xrt 处理疾病的数据
class RnaDiseaseProcessor(DataProcessor):
    """Processor for the ncRNA promoter data"""
    def get_labels(self):
        return ["0", "1"]
    def get_train_examples(self, data_dir):
...
glue_processors = {
    ...
    "rnafamily": RnaFamilyProcessor,
    "pre_train": PreTrainProcessor,
    "rnadisease": RnaDiseaseProcessor
}
...
glue_output_modes = {
    ...
    "rnafamily": "classification",
    "pre_train": "classification",
    "rnadisease": "classification"
}
