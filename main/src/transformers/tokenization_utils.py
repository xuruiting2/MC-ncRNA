...
class PreTrainedTokenizer(object):
        ...
        **kwargs
        ...
        def get_input_ids(text):
            if isinstance(text, str):
                # 这个只是将字符串变成了数组，但是个数没有改变，并且还是英文，还没有变成数字
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        # 在这创建一字典，作用是将当前序列的id和当前的向量对起来
        with open('/home/xuruiting/ncRNA_ss/DNABERT-master/examples/sample_data/rnaData_pre/result.json', 'r') as file:
            # 加载JSON数据
            result_data = json.load(file)
        id_seq_dict = {}
        # print(batch_text_or_text_pairs, '我是输入进来的')
        # 输入进来的是两万多数据，全部的数据
        # print(len(batch_text_or_text_pairs), '我是输入进来的长度')
        # sys.exit()
        """
        ...
        batch_outputs = {}
        # 这个是用来区分是json中的第几个的变量
        i = 0
        for ids_or_pair_ids in batch_text_or_text_pairs:
            # ids_or_pair_ids表示的是个单条的数据，这里的数据还没有经过处理，并且现在还是按照顺序来，这个的第一个就是我们数据集中的那个第一个
            # print(len(ids_or_pair_ids), 'ids_or_pair_ids长度')
            # print(ids_or_pair_ids, 'ids_or_pair_ids')
            # print(len(batch_text_or_text_pairs), 'batch_text_or_text_pairs')
            if isinstance(ids_or_pair_ids, (list, tuple)):
                # print(ids_or_pair_ids, '所以到底是不是')
                # sys.exit()
                ...
                # 这个pair_ids就是空的
                # print(ids, '我是ids')
                # print(pair_ids, '我是pair_dis')
                key_id = result_data[i]["number"]
                # print(key, '我看取到了没,我是key')
                # print(i, '我看取到了没，我是i')
                # sys.exit()
                id_seq_dict[key_id] = ids
                # print(id_seq_dict, '我是id_seq_dict[key]')
                # sys.exit()
            ...
            # 目前字典已经更新成id了
            # print(batch_outputs["input_ids"][i], '我是batch_outputs["input_ids"][i]')
            # print(i)
            # sys.exit()
            id_seq_dict[key_id] = batch_outputs["input_ids"][i]
            i += 1
            # if i == 5:
            #     print(id_seq_dict, '我是id_seq_dict')
            #     sys.exit()
            # print(batch_outputs, '我是batch_outputs')
            # sys.exit()

        # print(len(id_seq_dict), '我是id_seq_dict长度')
        # print(id_seq_dict, '我是id_seq_dict')
        # sys.exit()
        ...
        # print(id_seq_dict, '我是tokenization中的id_seq_dict')
        # print(len(id_seq_dict), '我是tokenization中的id_seq_dict的长度')
        # sys.exit()
        return batch_outputs, id_seq_dict
    ...
    def all_special_tokens(self):
        ...
        """
        all_toks = []
        # {'unk_token': '[UNK]', 'sep_token': '[SEP]', 'pad_token': '[PAD]', 'cls_token': '[CLS]', 'mask_token': '[MASK]'} set_attr
        set_attr = self.special_tokens_map
        for attr_value in set_attr.values():
            all_toks = all_toks + (list(attr_value) if isinstance(attr_value, (list, tuple)) else [attr_value])
        # ['[UNK]', '[PAD]', '[CLS]', '[MASK]', '[SEP]']
